...
class IpAddress {
	...
	inline bool isZero() const
	{
		...
		return ((m_ip.family == AF_INET && m_ip.ip_addr.ipv4_addr == 0) ||
			(m_ip.family == AF_INET6 &&
			 memcmp(&m_ip.ip_addr.ipv6_addr, zerobuf, 16) == 0));
	}
	...
	inline bool operator<(const IpAddress &o) const
	{
		...
		if (m_ip.family == AF_INET)
			return m_ip.ip_addr.ipv4_addr < o.m_ip.ip_addr.ipv4_addr;

		return (memcmp(&m_ip.ip_addr.ipv6_addr,
			       &o.m_ip.ip_addr.ipv6_addr, 16) < 0);
	}
	...
	inline bool operator==(const IpAddress &o) const
	{
		return m_ip.family == o.m_ip.family &&
		       ((m_ip.family == AF_INET &&
			 m_ip.ip_addr.ipv4_addr == o.m_ip.ip_addr.ipv4_addr) ||
			(m_ip.family == AF_INET6 &&
			 memcmp(&m_ip.ip_addr.ipv6_addr,
				o.m_ip.ip_addr.ipv6_addr, 16) == 0));
	}
	...
	enum AddrScope { GLOBAL_SCOPE, LINK_SCOPE, HOST_SCOPE, MCAST_SCOPE };
