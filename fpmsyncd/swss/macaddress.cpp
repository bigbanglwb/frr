...
const size_t mac_address_str_length =
	ETHER_ADDR_LEN * 2 +
	5; // 6 hexadecimal numbers (two digits each) + 5 delimiters
...
MacAddress::MacAddress(const std::string &macStr)
{
	...
	if (!suc)
		throw invalid_argument("can't parse mac address '" + macStr +
				       "'");
}
...
std::string MacAddress::to_string(const uint8_t *mac)
{
	...
	for (int i = 0; i < ETHER_ADDR_LEN; ++i) {
		...
		int right = left +
			    1; // right digit position of hexadecimal number
...
bool MacAddress::parseMacString(const string &str_mac, uint8_t *bin_mac)
{
	...
	if (!(ptr_mac[2] == ptr_mac[5] && ptr_mac[5] == ptr_mac[8] &&
	      ptr_mac[8] == ptr_mac[11] && ptr_mac[11] == ptr_mac[14])) {
		return false;
	}
	...
	for (int i = 0; i < ETHER_ADDR_LEN; ++i) {
		...
		if (ptr_mac[left] >= '0' && ptr_mac[left] <= '9') {
			...
			bin_mac[i] = static_cast<uint8_t>(ptr_mac[left] - 'A' +
							  0x0a);
		} else if (ptr_mac[left] >= 'a' && ptr_mac[left] <= 'f') {
			bin_mac[i] = static_cast<uint8_t>(ptr_mac[left] - 'a' +
							  0x0a);
		...
		if (ptr_mac[right] >= '0' && ptr_mac[right] <= '9') {
			...
			bin_mac[i] |= static_cast<uint8_t>(ptr_mac[right] -
							   'A' + 0x0a);
		} else if (ptr_mac[right] >= 'a' && ptr_mac[right] <= 'f') {
			bin_mac[i] |= static_cast<uint8_t>(ptr_mac[right] -
							   'a' + 0x0a);
