...
class IpPrefix {
	...
	inline IpAddress getBroadcastIp() const
	{
		...
		case AF_INET: {
			return IpAddress((m_ip.getV4Addr()) |
					 ~(getMask().getV4Addr()));
		}
		case AF_INET6: {
			ip_addr_t ipa;
			...
			for (int i = 0; i < 16; ++i) {
				ipa.ip_addr.ipv6_addr[i] = (uint8_t)(prefix[i] |
								     ~mask[i]);
			}
	...
	inline IpAddress getMask() const
	{
		switch (m_ip.getIp().family) {
		case AF_INET: {
			return IpAddress(htonl(
				(uint32_t)((0xFFFFFFFFLL << (32 - m_mask)) &
					   0xFFFFFFFFLL)));
		}
		case AF_INET6: {
			ip_addr_t ipa;
			...
			if (mid < 16) {
				...
				ipa.ip_addr.ipv6_addr[mid] =
					(uint8_t)(0xFF << (8 - bits));
				memset(ipa.ip_addr.ipv6_addr + mid + 1, 0,
				       16 - mid - 1);
			}
			return IpAddress(ipa);
		}
	...
	inline IpPrefix getSubnet() const
	{
		...
		case AF_INET6: {
			...
			std::string ipStr(inet_ntop(AF_INET6, &subnet, buf,
						    INET6_ADDRSTRLEN));
