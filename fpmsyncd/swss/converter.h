...
__to_uint64(const std::string &str,
	    ...
	    uint64_t max = std::numeric_limits<uint64_t>::max())
{
	...
	if (str[idx]) {
		throw std::invalid_argument("failed to convert " + str +
					    " value to uint64_t type");
	}

	if (ret < min || ret > max) {
		throw std::invalid_argument(
			"failed to convert " + str + " value is not in range " +
...
__to_int64(const std::string &str,
...
template <typename T>
T to_int(const std::string &str, T min = std::numeric_limits<T>::min(),
...
template <typename T>
T to_uint(const std::string &str, T min = std::numeric_limits<T>::min(),
	  T max = std::numeric_limits<T>::max())
{
