    ...
    Selectable(pri), m_socket(NULL)
{
    ...
    if (!m_socket)
    {
        zlog_err("Unable to allocated netlink socket");
    ...
    if (err < 0)
    {
        zlog_err("Unable to connect netlink socket: %s", nl_geterror(err));
...
void NetLink::registerGroup(int rtnlGroup)
{
    ...
    if (err < 0)
    {
        zlog_err("Unable to register to group %d: %s", rtnlGroup,
                       nl_geterror(err));
        throw system_error(make_error_code(errc::address_not_available),
...
void NetLink::dumpRequest(int rtmGetCommand)
{
    ...
    if (err < 0)
    {
        zlog_err("Unable to request dump on group %d: %s", rtmGetCommand,
                       nl_geterror(err));
        throw system_error(make_error_code(errc::address_not_available),
...
uint64_t NetLink::readData()
{
    ...
    {
        if (err == -NLE_NOMEM)
            zlog_err("netlink reports out of memory on reading a netlink socket. High possibility of a lost message");
        else if (err == -NLE_AGAIN)
            zlog_debug("netlink reports NLE_AGAIN on reading a netlink socket");
        else
            zlog_err("netlink reports an error=%d on reading a netlink socket", err);
    }
    return 0;
}
